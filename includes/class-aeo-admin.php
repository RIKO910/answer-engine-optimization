...
class AEO_Admin {
    ...
    public function general_section_callback() {
        echo __('Configure how your content is optimized for answer engines.', 'answer-engine-optimization');
    }

    public function checkbox_field_render($args) {
        $options = get_option('aeo_settings');
        ?>
        <input type="checkbox" name="aeo_settings[<?php echo $args['name']; ?>]" value="1" <?php checked(1, $options[$args['name']] ?? 0); ?>>
    ...
    public function textarea_field_render($args) {
        $options = get_option('aeo_settings');
        ?>
        <textarea name="aeo_settings[<?php echo $args['name']; ?>]" rows="5" cols="50"><?php echo $options[$args['name']] ?? ''; ?></textarea>
    ...
    public function settings_page() {
        ?>
        <div class="wrap">
            <h1><?php _e('Answer Engine Optimization Settings', 'answer-engine-optimization'); ?></h1>
    ...
    public function render_meta_box($post) {
        ...
        <div class="aeo-meta-box">
            <div class="aeo-field">
                <label for="aeo_target_question"><?php _e('Target Question', 'answer-engine-optimization'); ?></label>
                <input type="text" id="aeo_target_question" name="aeo_target_question" value="<?php echo esc_attr($target_question); ?>" class="widefat">
                <p class="description"><?php _e('The specific question this content answers (e.g., "How do I optimize for answer engines?")', 'answer-engine-optimization'); ?></p>
            </div>

            <div class="aeo-field">
                <label for="aeo_direct_answer"><?php _e('Direct Answer', 'answer-engine-optimization'); ?></label>
                <textarea id="aeo_direct_answer" name="aeo_direct_answer" rows="3" class="widefat"><?php echo esc_textarea($direct_answer); ?></textarea>
                <p class="description"><?php _e('A concise, 1-2 sentence answer to the target question', 'answer-engine-optimization'); ?></p>
            </div>

            <div class="aeo-field">
                <label><?php _e('FAQ Items', 'answer-engine-optimization'); ?></label>
                    ...
                    if (!empty($faq_items) && is_array($faq_items)) {
                        foreach ($faq_items as $index => $faq_item) {
                            ...
                            <div class="aeo-faq-item">
                                <input type="text" name="aeo_faq_question[]" value="<?php echo esc_attr($faq_item['question']); ?>" placeholder="<?php _e('Question', 'answer-engine-optimization'); ?>" class="widefat">
                                <textarea name="aeo_faq_answer[]" rows="2" placeholder="<?php _e('Answer', 'answer-engine-optimization'); ?>" class="widefat"><?php echo esc_textarea($faq_item['answer']); ?></textarea>
                ...
                <button type="button" id="aeo-add-faq" class="button"><?php _e('Add FAQ Item', 'answer-engine-optimization'); ?></button>
                <p class="description"><?php _e('Add questions and answers that are relevant to your content', 'answer-engine-optimization'); ?></p>
            </div>
        </div>
        <?php
    }
